...

{
  ...
  <<"\n\nDriver: \t"<<bus[m].bus_driver<<"\t\tBus_date: \t\t"<<bus[m].bus_date<<"\nArrival Time: \t"
...

{
...

  {
    ...
    cout<<"\nBus Number: \t"<<bus[y].bus_number<<"\n\nDriver: \t"<<bus[y].bus_driver<<"\t\tBus_date: \t\t"<<bus[y].bus_date
...

  {
    ...
    if (strcmp(bus[y].bus_seat[bus_seat/4][(bus_seat%4)+1], "Empty")==0)

      {
        ...
        cin>>bus[y].bus_seat[bus_seat/4][(bus_seat%4)+1];
        cout<<"YOUR TICKET CANCELLED SUCCESFULLY"<<endl;
      ...
      cout<<"The Seat Number. is Already cancelled.\n";
...

{
...

  {
	...
	case 5: bus[t].cancel_reservation();
